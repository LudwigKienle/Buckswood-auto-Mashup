...
export function TrackPanel({letter,track,tracks,onChoose,onUpload,busy,lalalReady}){
 ...
 return <section className={'track track-'+letter} aria-label={'Track '+letter}>
  ...
  {tracks.length>0?<select aria-label={'Song für Track '+letter} className="track-select" disabled={busy} value={track?.id||''} onChange={e=>{setStem('audio');onChoose(e.target.value)}}><option value="" disabled>Song wählen</option>{tracks.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select>:<p>Wähle deinen {letter==='A'?'ersten':'zweiten'} Song.</p>}
  ...
  {track?<audio ref={player} key={track.id+stem} controls preload="metadata" onLoadedMetadata={()=>{const audio=player.current;if(!audio)return;audio.currentTime=Math.min(audition.current.at,audio.duration||0);if(audition.current.playing)audio.play().catch(()=>{});}} src={stem==='audio'?`/api/tracks/${track.id}/audio`:`/api/tracks/${track.id}/stems/${stem==='vocals-old'||stem==='vocals-lalal'?'vocals':stem}?quality=${stem==='vocals-lalal'?'lalal':stem==='vocals-old'?'standard':'auto'}`}/>:<div className="audio-empty">MP3 · WAV · OGG · FLAC · M4A · AIFF</div>}
  <div className="track-bottom"><button disabled={busy} onClick={()=>file.current.click()}>↥ &nbsp; Datei wählen</button><input ref={file} hidden type="file" accept="audio/*,.ogg,.flac" onChange={e=>{if(e.target.files[0])onUpload(e.target.files[0]);e.target.value=''}}/>
   {(track?.separated||track?.hq_separated||lalalReady)&&<select aria-label={'Spur vorhören Track '+letter} value={stem} onChange={e=>selectStem(e.target.value)}><option value="audio">Original</option>{lalalReady&&<option value="vocals-lalal">Gesang · LALAL.AI</option>}<option value="vocals">{track.hq_separated?'Gesang · RoFormer':'Gesang solo'}</option>{track.hq_separated&&track.separated&&<option value="vocals-old">Gesang · Demucs bisher</option>}<option value="drums">Drums solo</option><option value="bass">Bass solo</option><option value="other">Instrumente solo</option></select>}
  </div>
 </section>
}
