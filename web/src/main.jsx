...
function App(){
 ...
 async function upload(file,side){setRequesting(true);setError('');try{const form=new FormData();form.append('file',file);setImportSide(side);setJob(await api('/upload',form));}catch(e){setError(e.message)}finally{setRequesting(false)}}
