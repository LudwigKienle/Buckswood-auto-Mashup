...
function App(){
 ...
 useEffect(()=>{(async()=>{try{const state=await refresh();let saved=null;try{saved=JSON.parse(localStorage.getItem('mashup-session'));}catch{}
  ...
  if(valid){setSections(saved.sections||[]);setBpm(saved.bpm??'');setKeyMatch(saved.keyMatch??true);setBpmA(saved.bpmA??'');setBpmB(saved.bpmB??'');setPitchB(saved.pitchB??'');setQuality(saved.quality??null);setSeparationQuality(saved.separationQuality??'auto');setProtectPhrases(saved.protectPhrases??true);}else await plan(x,y);
 ...
 useEffect(()=>{if(!loading)localStorage.setItem('mashup-session',JSON.stringify({a,b,sections,bpm,keyMatch,bpmA,bpmB,pitchB,quality,separationQuality,protectPhrases}));},[a,b,sections,bpm,keyMatch,bpmA,bpmB,pitchB,quality,separationQuality,protectPhrases,loading]);
 ...
 async function introPlan(){setRequesting(true);setError('');try{const next=await api('/intro-plan',{track_a:a,track_b:b,sections,bpm_a:bpmA===''?null:Number(bpmA),bpm_b:bpmB===''?null:Number(bpmB)});setSections(next.sections);setQuality(null);}catch(e){setError(e.message)}finally{setRequesting(false)}}
 async function qualityPlan(){setRequesting(true);setError('');try{setJob(await api('/quality-plan',{track_a:a,track_b:b,target_bpm:bpm===''?null:Number(bpm)}));}catch(e){setError(e.message)}finally{setRequesting(false)}}
